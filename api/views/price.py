    ...
    def perform_create(self, serializer):
        ...
        if date > timezone.now().date():
            raise serializers.ValidationError(
                {"error": "Price date cannot be in the future."}
            )
...
def calculate_average_price(request):
    ...
    if not all([product_id, start_date_str, end_date_str]):
        return Response(
            {"error": "Please provide product_id, start_date, and end_date."},
    ...
    average_price = Price.objects.filter(
        product=product, date__range=(start_date, end_date)
    ).aggregate(Avg("price"))["price__avg"]

    if average_price is None:
        return Response(
            {"message": "No prices found for the given period."},
